...
# https://raw.githubusercontent.com/msracver/Flow-Guided-Feature-Aggregation/master/lib/nms/seq_nms.py
NMS_THRESH = 0.3
IOU_THRESH = 0.5
MAX_THRESH=1e-2
...
# Junwei Liang, 03/2019 for diva
# dets are num_class -> num_frames
def seq_nms(dets, CLASSES):
    links = createLinks(dets, CLASSES)
    dets = maxPath(dets, links)
    # dets is num_class -> num_frame
    num_frame = len(dets[0])
    newboxes = [[] for i in xrange(num_frame)]
    newclasses = [[] for i in xrange(num_frame)]
    newscores = [[] for i in xrange(num_frame)]
    for cls_idx, det in enumerate(dets):
        for frame_idx, d in enumerate(det):
            # d is [num_box, 5]
            newboxes[frame_idx]+=[list(d[i, :4]) for i in xrange(len(d))]
            newscores[frame_idx]+=[d[i, 4] for i in xrange(len(d))]
            newclasses[frame_idx]+=[cls_idx+1 for i in xrange(len(d))]
    return newboxes, newclasses, newscores

def seq_nms2(dets, CLASSES):
    links = createLinks(dets, CLASSES)
    dets = maxPath(dets, links)
    # dets is num_class -> num_frame -> [N, 5]
    return dets